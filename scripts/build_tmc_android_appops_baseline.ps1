...
$appRoot = Join-Path $repoRoot 'benchmarks\android_appops_baseline_app'
$buildRoot = Join-Path $appRoot 'build'
$classes = Join-Path $buildRoot 'classes'
$dex = Join-Path $buildRoot 'dex'
$unsigned = Join-Path $buildRoot 'appops-baseline-unsigned.apk'
$aligned = Join-Path $buildRoot 'dros-tmc-appops-baseline.apk'
$keystore = Join-Path $buildRoot 'debug.keystore'
$source = Join-Path $appRoot 'src\io\dros\tmc\appopsbaseline\MainActivity.java'
...
& (Join-Path $buildTools 'd8.bat') --lib $androidJar --output $dex (Join-Path $classes 'io\dros\tmc\appopsbaseline\MainActivity.class')
