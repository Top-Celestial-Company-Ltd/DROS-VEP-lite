"""Run the single ANDROID-BASE-03 authorized path through a real AVD."""
...
def build_authorized_request() -> dict[str, object]:
    """Return the BASE-03 request with the one intended authority change."""

    return {
        "request_id": "ANDROID-BASE-03-000001",
        "principal": "authorized-agent",
        "action": "READ_CONTACTS",
        "capability": "CAP_CONTACTS",
        "policy_version": "android-baseline-policy-v1",
        "revocation_epoch": 0,
        "arguments_hash": "sha256:android-base-03-args",
    }
...
def run(args: argparse.Namespace) -> int:
    ...
    summary = {
        ...
        "measurement_scope": "security ordering and bounded fixture effect only; no performance or energy claim",
    }
    ...
    if result.get("decision") != "ALLOW":
        return 1
    if result.get("execution_started") is not True:
        return 1
    if result.get("execution_completed") is not True:
        return 1
    if result.get("execution_effect") != "BOUNDED_FIXTURE":
        return 1
