...
public final class MainActivity extends Activity {
    ...
    private JSONObject evaluate(String raw) {
        ...
        try {
            ...
            result.put("authority", "ANDROID_APPOPS_READ_CONTACTS");

            AppOpsManager appOps = (AppOpsManager) getSystemService(APP_OPS_SERVICE);
            int mode = appOps.unsafeCheckOpNoThrow(
                    AppOpsManager.OPSTR_READ_CONTACTS, Process.myUid(), getPackageName());
            if (mode != AppOpsManager.MODE_ALLOWED) {
                return finishResult(result, "DENY", false, false, "NONE",
                        "ANDROID_APPOPS_DENIED", started);
            }
            ...
            result.put("execution_effect", "BOUNDED_APPOPS_FIXTURE");
            result.put("reason", "ANDROID_APPOPS_ALLOWED");
            result.put("fixture_rows_observed", rows);
            appendEffect(new JSONObject()
                    .put("request_id", requestId)
                    .put("effect", "APPOPS_CONTACTS_PROVIDER_QUERY")
