...
在自主 AI 工作負載中，受陷應用層可繞過語義防火牆執行特權操作。DROS 提出四層執行期作業基板與確定性執行強制機制，將 Agent 身分與執行授權解耦，並在受控二進位 C-ABI / FFI 邊界強制執行授權點陣圖。本文提出漸進式對抗驗證體系，整合 Strix 自主黑客、變形測試、帶外神諭與開源反例搜尋，在實例化評測空間中維持未授權物理執行 $I_{\text{physical}} = 0$。

**關鍵字：** AI Agent 安全、執行期作業基板、受控 C-ABI 授權邊界、自主對抗性評測、RCU 原子狀態切換、動態資訊流控制 (IFC)、歸因治理。
