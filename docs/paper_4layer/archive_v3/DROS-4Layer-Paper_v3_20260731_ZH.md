...
### 5.4 微觀基準量測數據 (24 小時實體浸泡測試)

| 評測指標 | 24 小時實測數據 (Empirical Value) | 標準差 / 備註 |
| :--- | :--- | :--- |
| **總測試請求負載** | **160,611 次** | 連續 24.0 小時不間斷執行 |
| **策略評估延遲 (P50)** | **26.21 μs (0.0262 ms)** | $\pm 0.34\ \mu\text{s}$ (極致穩定) |
| **P99 策略延遲** | **242.69 μs (0.242 ms)** | 高並發最高峰值延遲 |
| **C-ABI 實體熔斷延遲** | **< 500 ns** | $\pm 42\ \text{ns}$ |
| **SPEC CPU2017 運行負載** | **< 1.8%** | 極低核心上下文切換 |
| **確定性阻斷成功率** | **100% (0 系統外洩)** | 成功實體阻斷 137,751 次惡意攻擊 |
...
## 七、 結語與未來展望 (Conclusion & Future Work)

隨著自主型 AI 工作負載在企業核心基礎設施中承擔關鍵運作角色，僅依賴機率性應用層防火牆已無法單獨提供完備的執行安全保證。本論文提出 DROS 四層縱深防禦架構，建立了一個結合三階 PKI 身分驗證與亞微秒級 C-ABI 二進位邊界強制（<500ns 熔斷延遲，26.1μs 中位數策略評估延遲）的確定性運行期控制面。

透過將執行能力隔離於不可變之點陣圖邊界，並經由簽署之 `DrosIdentityToken (DIT)` 驗證身分，DROS 有效解決了作業系統層級的歸責缺口，並能實體封鎖包含零日間接提示詞注入（IPI）與跨企業 B2B 供應鏈感染在內的後劫持攻擊向量。在連續 24 小時不間斷之基準測試中，實證顯示本架構在無需修改底層 LLM 模型架構或應用程式碼的前提下，實現了零負載擴充性與 100% 確定性物理阻斷。未來研究將專注於擴展多 Agent 間記憶共享協定的動態形式化驗證（Formal Verification）。
