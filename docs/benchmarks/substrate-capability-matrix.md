...
| **Principal Attribution (PC-010)** | ✅ Native (DENY) | ✅ Native (DENY) | ❌ Unsupported | ❌ Unsupported | ❌ Unsupported | ❌ Unsupported | 📐 Model Invariant |
| **Task Authorization** | ✅ Native (DENY) | ✅ Native (DENY) | ❌ Unsupported | ❌ Unsupported | ❌ Unsupported | ❌ Unsupported | 📐 Model Invariant |
| **Tool / Action Binding (PC-004)** | ✅ Native (DENY) | ✅ Native (DENY) | ❌ Unsupported | ⚠️ Endpoint missing (DENY) | ❌ Unsupported | ❌ Unsupported | 📐 Model Invariant |
| **Argument Semantic Bounds (PC-005)** | ✅ Native (DENY) | ✅ Native (DENY) | ❌ Unsupported | ❌ Unsupported | ❌ Unsupported | ❌ Unsupported | 📐 Model Invariant |
...
| **Temporal Expiry [TTL] (PC-007)** | ✅ Dynamic timer (DENY) | ✅ Dynamic timer (DENY) | ❌ Unsupported | ❌ Unsupported | ❌ Unsupported | ❌ Unsupported | 📐 Model Invariant |
| **Hot Revocation (PC-008)** | ✅ In-band state (DENY) | ✅ In-band state (DENY) | ❌ Unsupported | ⚠️ seL4_CNode_Revoke (DENY)** | ❌ Unsupported (No HW temporal revoke) | ⚠️ CheriBSD temporal sweep (DENY) | 📐 Model Invariant |
| **Replay / Nonce Protection (PC-009)** | ✅ Nonce cache (DENY) | ✅ Nonce cache (DENY) | ❌ Unsupported | ❌ Unsupported | ❌ Unsupported | ❌ Unsupported | 📐 Model Invariant |
...
1. **`Unsupported` is Not a Security Failure**:
   - Marking `WASI`, `seL4`, or `CHERI` as `Unsupported` for argument-level semantics (PC-005) or temporal token expiry (PC-007) reflects their true architectural abstraction (operating on descriptors, capabilities, and memory bounds, rather than LLM application JSON structures).
...
3. **Hardware Architecture vs. Runtime OS (CHERI vs. CheriBSD)**:
   - Pure CHERI ISA architecture possesses no concept of temporal token revocation (PC-008 = `UNSUPPORTED`). When paired with CheriBSD's temporal safety runtime (Cornucopia), heap revocation is performed via memory sweeps.
